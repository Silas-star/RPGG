...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiin @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*Mande !menu pra saber de coisas como a ficha,nerfs,hist�rias e sistemas e mais op��es lembre-se de se registrar no !registrar {seu nomer}|{sua idade} para usar o menu`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Processando caso n�o for manda de novo ⌛',
				success: '✔️ Sucesso ✔️',
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo Silas! ❌',
			...
			const ownerNumber = ["554898006626@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '554898006626@s.whatsapp.net'
                        /******ApiKey Input******/
